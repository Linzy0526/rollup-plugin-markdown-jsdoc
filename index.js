...
function markdownJsdocPlugin(options = {}) {
    ...
    return {
        ...
        transform(code, id) {
            const isExcluded = exclude.some(pattern => checkPath(id, pattern));
            const isIncluded = include.length === 0 || include.some(pattern => checkPath(id, pattern));
        ...
        writeBundle() {
            generateDoc(results, outputPath);
        }
    }
}
